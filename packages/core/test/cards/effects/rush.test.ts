...
describe('RUSH effect handler', () => {
  beforeEach(() => {
    clearEffectHandlers();
  });

  it('registers RUSH handler (empty implementation) without error', () => {
    ...
    const rushHandler = handlers.find(h => h.keyword === 'RUSH');
    expect(rushHandler).toBeDefined();
    expect(rushHandler!.keyword).toBe('RUSH');
  });

  it('resolveEffects does not throw with registered RUSH handler', () => {
    const handler: EffectHandler = {
      keyword: 'RUSH',
    };
    registerEffectHandler(handler);

    const source = makeCardInstance({ card: makeCard('test_rush', ['RUSH']) });
