...
describe('TAUNT effect handler', () => {
  beforeEach(() => {
    clearEffectHandlers();
  });

  it('registers TAUNT handler (empty implementation) without error', () => {
    ...
    const tauntHandler = handlers.find(h => h.keyword === 'TAUNT');
    expect(tauntHandler).toBeDefined();
    expect(tauntHandler!.keyword).toBe('TAUNT');
  });

  it('resolveEffects does not throw with registered TAUNT handler', () => {
    const handler: EffectHandler = {
      keyword: 'TAUNT',
    };
    registerEffectHandler(handler);

    const source = makeCardInstance({ card: makeCard('test_taunt', ['TAUNT']) });
    const ctx = makeEffectContext({ source });

    // TAUNT has no lifecycle hooks, so resolveEffects should be a no-op
